...
class Cave:
    ...
    def __init__(self, paths):
        ...
        self.grid = np.zeros(shape=(self.bottom, self.right))
    ...
    def can_move(self, pt):
        ...
        if self._is_abyss_pathway(down):
            raise Exception("abyss")

        if not self.is_occupied(down):
            return down

        if self._is_abyss_pathway(down_left):
            raise Exception("abyss")

        if not self.is_occupied(down_left):
            return down_left

        if self._is_abyss_pathway(down_right):
            raise Exception("abyss")
    ...
    def run(self, n):
        ...
        for i in range(n):
            try:
                ...
                self.remove(self.current_grain_location)
                new_pos = None
                return
    ...
    def _is_abyss_pathway(self, pt):
        """returns True if the grain at point pt is in a position where it would fall into the abyss
        """
        # first check if the pt.c is outside of the cave boundry, in which case it definately will
        # fall into the abyss
        # next check if the pt.r position is below the cave boundry, and ditto
        return pt.c <self.left or pt.c >= self.right or pt.r > self.bottom

    def is_occupied(self, pt):
        return self.grid[pt.r, pt.c] != 0
